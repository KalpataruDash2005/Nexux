    ...
    return (
      <div className="phq-fade-in mx-auto max-w-2xl">
        <Card>
          ...
          <h2 className="text-2xl font-extrabold tracking-tight text-foreground">Aptitude Test</h2>
          <p className="mt-1 text-sm text-muted">Sharpen quantitative, logical, verbal and puzzle-solving skills with timed AI-generated tests.</p>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted">Number of questions</label>
              <div className="grid grid-cols-3 gap-2">
                {COUNTS.map((c) => (
                  <button
                    key={c}
    ...
    return (
      <div className="phq-fade-in mx-auto max-w-3xl">
        <Card className="text-center">
          ...
          <h2 className="text-xl font-extrabold text-foreground">Test complete!</h2>
          ...
          <button
            onClick={handleReset}
            className="mt-6 flex items-center justify-center gap-2 rounded-xl bg-primary hover:bg-primary-hover px-6 py-2.5 text-sm font-bold text-white shadow-card transition-all hover:shadow-lg"
          >
            <RotateCcw size={15} />
            New test
          </button>
                  ...
                  <div className="flex-1">
                    ...
                    {question?.options.map((opt, oi) => (
                      ...
                      >
                        ...
                        {oi === d.correctAnswerIndex && <span className="ml-2 text-[10px] font-bold uppercase text-emerald-600">Correct</span>}
                        {oi === d.yourAnswer && !d.correct && <span className="ml-2 text-[10px] font-bold uppercase text-rose-600">Your answer</span>}
                      </div>
                    ))}
                    {d.explanation && (
                      <p className="mt-2 rounded-lg border border-border bg-tag-bg p-3 text-sm text-muted">{d.explanation}</p>
                    )}
                  </div>
                </div>
              </Card>
    ...
    <div className="phq-fade-in mx-auto max-w-3xl">
      <Card>
        ...
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-bold text-foreground">Question {current + 1} / {questions.length}</p>
        ...
        <div className="h-1.5 overflow-hidden rounded-full bg-tag-bg">
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm transition-all ${
                    selected
                      ? 'border-primary-soft bg-primary-soft text-foreground shadow-card'
                      : 'border-border text-foreground hover:border-primary-soft hover:bg-tag-bg'
                  }`}
                >
                  <span
        ...
        <div className="mt-6 flex items-center justify-between gap-3">
          <button
            ...
            disabled={current === 0}
            className="flex items-center gap-1.5 rounded-xl border border-border px-4 py-2.5 text-sm font-semibold text-muted transition-colors hover:border-primary-soft disabled:opacity-40"
          >
            <ChevronLeft size={16} />
            Previous
          </button>
            ...
            {questions.map((_, i) => (
              <button
                ...
                className={`h-2.5 w-2.5 rounded-full transition-all ${
                  i === current ? 'bg-primary scale-125' : answers[questions[i].id] !== undefined ? 'bg-primary-soft' : 'bg-border'
                }`}
              />
            ))}
          </div>
          {current < questions.length - 1 ? (
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl border border-border px-4 py-2.5 text-sm font-semibold text-muted transition-colors hover:border-primary-soft"
            >
              Next
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-primary hover:bg-primary-hover px-5 py-2.5 text-sm font-bold text-white shadow-card transition-all hover:shadow-lg disabled:opacity-50"
            >
